##比较WT小鼠TAC之后的基因表达的变化的情况 down WT< sham up WT > sham
rm(list = ls())
...
#比较WS vs WT
filter_count=filter_count[,c(7,8,9,10,11,12)]
group= group[c(7,8,9,10,11,12),]
...
comp <- unlist(strsplit("WT_vs_WS", split = "_vs_"))
...
DEG_edgeR_symbol_up <- filter(DEG_edgeR_symbol,regulated=="up")
DEG_edgeR_symbol_down <- filter(DEG_edgeR_symbol,regulated=="down")

# 保存
write.csv(DEG_edgeR_symbol,"Data/WTvsWS_all.csv", row.names = F)
write.csv(DEG_edgeR_symbol_Sig,"Data/WTvsWS_Sig.csv", row.names = F)
write.csv(DEG_edgeR_symbol_up,"Data/WTvsWS_Sig_up.csv", row.names = F)
write.csv(DEG_edgeR_symbol_down,"Data/WTvsWS_Sig_down.csv", row.names = F)

save(DEG_edgeR_symbol,DEG_edgeR_symbol_Sig,DEG_edgeR_symbol_up, DEG_edgeR_symbol_down,file = "Data/Step03-WTvsWS.Rdata")
...
exp <- c(t(express_cpm[match("ENSMUSG00000000120",rownames(express_cpm)),]))
