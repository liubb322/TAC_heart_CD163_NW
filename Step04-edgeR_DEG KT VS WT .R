...
#比较TAC KT vs WT
filter_count=filter_count[,c(4,5,6,10,11,12)]
group= group[c(4,5,6,10,11,12),]
...
comp <- unlist(strsplit("WT_vs_KT", split = "_vs_"))
...
# 保存
write.csv(DEG_edgeR_symbol,"Data/KTvsWT_all.csv", row.names = F)
write.csv(DEG_edgeR_symbol_Sig,"Data/KTvsWT_Sig.csv", row.names = F)
write.csv(DEG_edgeR_symbol_up,"Data/KTvsWT_Sig_up.csv", row.names = F)
write.csv(DEG_edgeR_symbol_down,"Data/KTvsWT_Sig_down.csv", row.names = F)
...
exp <- c(t(express_cpm[match("ENSMUSG00000000031",rownames(express_cpm)),]))
